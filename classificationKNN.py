...
#wybranie zdjęcia [0] - to zdjęcie będzie testowane, a pozostałe będą służyły uczeniu modelu
images[0], images[5] = images[5], images[0]
done[0], done[5] = done[5], done[0]
...
for i in range(3, len(images[id])-3):
    ...
    for j in range(3, len(images[id][i])-3):
        ...
        k = 50
        prediction = knn.predict([properties])
        imageCopy[i][j] = prediction[0]
        q +=1
        if done[id][i][j][0] == prediction[0]:
                    accuracy += 1
        if (prediction[0] == 255 and done[id][i][j][0] == 255) or (prediction[0] == 0 and done[id][i][j][0] == 255):
                    sensitivity += 1
        if (prediction[0] == 255 and done[id][i][j][0] == 0) or (prediction[0] == 0 and done[id][i][j][0] == 0):
                    specificity += 1
        if (prediction[0] == 255 and done[id][i][j][0] == 255):
            tp+=1
        if (prediction[0] == 0 and done[id][i][j][0] == 255):
            fn+=1
        if (prediction[0] == 0 and done[id][i][j][0] == 0):
            tn+=1
        if (prediction[0] == 255 and done[id][i][j][0] == 0):
            fp+=1
